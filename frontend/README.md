...
- `/quests`, `/quests/new`, `/quests/[questId]`에서 본인 개인 퀘스트 초안을 생성·조회·수정·폐기합니다. 제목·조건 등 모든 항목을 입력해야 저장할 수 있고, 자유 입력 난이도는 1~4, 제안 코인은 0/100C, 인증 방식은 사용 안 함입니다. 제출·승인·튜토리얼 정산·사진 업로드는 이번 범위에 포함하지 않습니다.
...
- 연결 종료·차단 시 상대 정보를 즉시 숨기고 정리 상태를 조회합니다. `내 초안 정리 X / Y`는 본인 소유 초안 수만 뜻하며, 전체 작업이 `COMPLETED`가 되어야 재연결할 수 있습니다. 정리 중에는 앱 전경에서만 5초 간격으로 상태를 확인합니다.
...
미리보기는 계정 활성화·커플 연결·배츄 저장·코인 지급을 수행하지 않습니다. 앱을 새로 열면 미리보기 선택과 이름은 초기화됩니다. 퀘스트 제출·파트너 승인·정산·지갑 거래 내역은 후속 구현 범위입니다. 팔레트와 알 일러스트는 최종 아트 납품 전 사용하는 개발용 디자인입니다.
