...
1. 내 몬스터 상태
2. 현재 코인
3. 승인 대기·진행 중·결과 대기 건수와 처리해야 할 퀘스트
4. 파트너와의 비교
...
| 스타팅 배츄 선택 | 몬스터 외형 선택              |
| 홈         | 몬스터, 상태별 퀘스트 건수, 파트너 비교, 처리 대기함 |
...
`home`의 `questStatusSummary`는 `approvalPendingCount`, `activeCount`, `resultPendingCount`를 제공한다. 각각 사용자가 참여한 퀘스트 중 `PENDING_APPROVAL`, `ACTIVE`, `AWAITING_RESULT + PENDING_FINAL_APPROVAL` 상태를 집계한다. 건수 카드와 퀘스트 목록은 같은 상태·역할 투영기를 사용해 서로 다른 숫자를 보여주지 않아야 한다.
